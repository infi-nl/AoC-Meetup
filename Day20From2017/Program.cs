...
".Trim();
        ...
        static void Main(string[] args)
        {
             ...
             for (int i = 0; i < 1_000_000; i++)
             {
                 ...
                 particles = particles.GroupBy(p => p.Position)
                 ...
                 if (i % 50 == 0)
                 {
                     var closest = particles.Select(p => (p.Index, p.ManhattanDistance()))
                         .OrderBy(p => p.Item2)
                         .First();


                     Console.WriteLine($"{i} Particle {closest.Index} is closest with distance {closest.Item2} with {particles.Count} particles");
                 }
             }
        }
    }
    ...
    internal class Particle
    {
        ...
        public static Particle Deserialize(string input, int index)
        {
            var parts = input.Split(", ")
                .Select(part => part.Substring(3, part.Length - 4)
                    .Split(",")
                    .Select(long.Parse)
                )
                .Select(l =>
                {
                    var list = l.ToList();
                    return (X: list[0], Y: list[1], Z: list[2]);
                })
                .ToList();
